...
<html lang="en" ng-app = "myapp" ng-controller = "mycontroller">
  ...
  <body style="margin-left: 250px; margin-top: 80px;">
    <div class="container-fluid px-4">
        <h1 class="mt-4">Danh Sách Chi Tiết Hóa Đơn Bán</h1>
    ...
    <table class="table table-bordered">
        ...
        <tbody>
            <tr dir-paginate = "sp in CThoadonbans| filter: timkiem|itemsPerPage:10">
                <td>@{{ $index+1 }}</td>
...
<div class="modal-body">
                    ...
                    <div class="form-group">
                      <label for="id_bill_ban">Mã Hóa Đơn Bán</label>
                      <input type="text"
                        class="form-control" id="id_bill_ban" ng-model="CThoadonban.id_bill_ban">
                    </div>
                
                <div class="form-group">
                        <label for="id_sp">Mã Sản Phẩm</label>
                        <input type="text"
                          class="form-control" id="id_sp" ng-model="CThoadonban.id_sp">
                      </div>
                <div class="form-group">
                        <label for="sl">Số Lượng</label>
                        <input type="text"
                          class="form-control" id="sl" ng-model="CThoadonban.sl">
                      </div>
    ...
    <script>
        ...
        app.controller('mycontroller',function($scope,$http){
            ...
            $scope.reloadData = function(){
                    ...
                    {
                        method: "GET",
                        url: "http://localhost:8000/api/CThoadonbans"
                    }
                ).then(function(response){
                    $scope.CThoadonbans = response.data;
                });
            }
            $scope.reloadData();
            ...
            $scope.showModal = function(id){
                $scope.id=id;
                if(id==0){
                    $scope.modalTitle = "Thêm chi tiết hóa đơn bán";
                    if($scope.CThoadonban) delete $scope.CThoadonban;
                }
                else{
                    ...
                    $http(
                        {
                            method: "GET",
                            url: "http://localhost:8000/api/CThoadonbans/"+id
                        }
                    ).then(function(response){
                        //console.log(response.data);
                        $scope.CThoadonban = response.data;
                    });
}
                $('#modelId').modal('show');
            }
            $scope.saveData = function(){
                if($scope.id==0){
                    ...
                    $http(
                        {
                            method: "PUT",
                            url: "http://localhost:8000/api/CThoadonbans/"+$scope.id,
                            data: $scope.CThoadonban,
                            "content-Type": "application/json",
                        }
                    ).then(function(response){
